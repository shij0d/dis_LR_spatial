...
def estimation(r,m=60,if_optimal=True,J=16,nu=1.5):
    ...
    random.seed(SEED+1)
    knots_inital = random.sample(locations, initial_m)
    #knots_inital=knots
    ...
    return elapsed_time_mle,elapsed_time_de,optimal_estimator[3:6],de_estimators[3:5]
...
for m in ms:
    ...
    for r in range(5):
        ...
        if m<=150:
            ...
            result=estimation(r,m,if_optimal=False) 
        results_r.append(result)
    results.append(results_r)
    path_save=os.path.join(path_project,'real_data/Second_scenario/time_com_varying_m/initial_fixed/time_com_varying_m_fixed_J.pkl')
