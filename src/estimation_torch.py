...
class GPPEstimation:
    ...
    def ce_optimize_stage2_shareK(self,mu:torch.Tensor,Sigma:torch.Tensor,beta:torch.Tensor,delta:torch.Tensor,theta:torch.Tensor,T:int,job_num,seed=2024,thread_num=None):
        torch.manual_seed(seed)
        if thread_num!=None:
            torch.set_num_threads(thread_num)
        #define some functions
        ...
        def y_mu_f_parallel_shareK(beta, theta):
            y_mu_M = torch.zeros((self.m, 1), dtype=torch.double)
            ...
            # Parallel execution for each j
            results = Parallel(n_jobs=job_num)(delayed(compute_j)(j) for j in range(self.J))
        ...
        def y_Sigma_f_parallel_shareK(theta):
            y_Sigma_M = torch.zeros((self.m, self.m), dtype=torch.double)
            K = K_f(theta)
            invK=torch.linalg.inv(K)
            def compute_j(j):
        ...
        def y_beta_f_parallel_shareK(mu, theta):
            y_beta_M = torch.zeros((self.p, 1), dtype=torch.double)
            K = K_f(theta)
            invK=torch.linalg.inv(K)
            def compute_j(j):
                ...
                return local_X.T @ (local_z - local_B @ mu)
        ...
        def y_delta_f_parallel_shareK(mu, Sigma, beta, theta):
            y_delta = torch.zeros((1,1), dtype=torch.double)
            K = K_f(theta)
            invK=torch.linalg.inv(K)
            ...
            def compute_j(j):
                ...
                term1 = torch.trace(local_B.T @ local_B @ (Sigma + mu @ mu.T))
        ...
        def y_value_f_parallel_shareK(mu,Sigma,beta,delta,theta):
            y_value_M=torch.zeros((1,1),dtype=torch.double)
            knots=self.knots
            K = self.kernel(knots, knots, theta)
            invK=torch.linalg.inv(K)
            def compute_j(j):
                ...
                local_B = self.kernel(local_locs, knots, theta) @ invK
                
                # Compute the value of the local objective function
                local_errorV= local_X @ beta-local_z
                local_value = -n * torch.log(delta) + delta * (
        ...
        def y_theta_f_parallel_shareK(mu, Sigma, beta, delta, theta):
            ...
            # Parallelize the computation using joblib
            results = Parallel(n_jobs=job_num)(delayed(compute_local_grad)(j) for j in range(self.J))

            # Populate y_theta_Mstack with the results
            for local_g_theta in results:
                y_theta_M += local_g_theta
            y_theta_M=y_theta_M/self.J
            return y_theta_M  
        ...
        def y_hessian_theta_f_parallel_shareK(mu, Sigma, beta, delta, theta):
            y_theta_M = torch.zeros((2, 2), dtype=torch.double)  # Hessian is a 2x2 matrix (second-order derivatives)
            knots = self.knots
            if theta.grad is not None:
                    theta.grad.data.zero_()
            theta.requires_grad_(True)
            ...
            def compute_local_hessian(j):
                ...
                grad = torch.autograd.grad(f_value, theta, create_graph=True)[0]  # Create graph to compute Hessian

                # Now compute the Hessian (second-order derivatives)
                hessian = torch.zeros((theta.numel(), theta.numel()), dtype=torch.double)
                for idx in range(theta.numel()):
                    # For each component of the gradient, compute the gradient again (second-order derivative)
                    grad2 = torch.autograd.grad(grad[idx], theta, retain_graph=True)[0]
                    hessian[idx, :] = grad2
                
                return hessian
    
            # Parallelize the computation using joblib
            results = Parallel(n_jobs=job_num)(delayed(compute_local_hessian)(j) for j in range(self.J))

            # Populate y_theta_Mstack with the results
            for local_hessian in results:
                y_theta_M += local_hessian
            
            y_theta_M = y_theta_M / self.J  # Average the Hessian over all tasks
        ...
        for t in range(T):
            ...
            #print(f"iteration:{t}")
            #mu and Sigma
            y_mu=y_mu_f_parallel_shareK(beta_list[t],theta_list[t])
            y_Sigma=y_Sigma_f_parallel_shareK(theta_list[t])
            K=K_f(theta_list[t])
            invK=torch.linalg.inv(K)
            Sigma=torch.linalg.inv(delta_list[t]*self.J*y_Sigma+invK)
            mu=torch.linalg.inv(delta_list[t]*y_Sigma+invK/self.J)*delta_list[t]@y_mu
            ...
            #beta
            y_beta=y_beta_f_parallel_shareK(mu,theta_list[t])
            beta=torch.linalg.inv(y_XTX)@y_beta
            beta_list.append(beta)


            #delta
            y_delta=y_delta_f_parallel_shareK(mu,Sigma,beta_list[t+1],theta_list[t])
            delta=size/y_delta
            delta_list.append(delta)
            ...
            theta=theta_list[t]
            for s in range(S):
                y_theta=y_theta_f_parallel_shareK(mu,Sigma,beta_list[t+1],delta_list[t+1],theta)
                y_hessian_theta=y_hessian_theta_f_parallel_shareK(mu,Sigma,beta_list[t+1],delta_list[t+1],theta)
                ...
                m=0.1
                Continue=True
                f_value=y_value_f_parallel_shareK(mu,Sigma,beta_list[t+1],delta_list[t+1],theta)*self.J+com_value_f(mu,Sigma,theta)
               
                while Continue:
                    theta_new=theta-step_size*invh_m_grad
                    if torch.all(theta_new>0.005):
                        f_value_new=y_value_f_parallel_shareK(mu,Sigma,beta_list[t+1],delta_list[t+1],theta_new)*self.J+com_value_f(mu,Sigma,theta_new)
                        if f_value_new>f_value-m*step_size*torch.dot(grad.squeeze(),invh_m_grad.squeeze()):
                           step_size*=shrink_rate
                        else:
                           theta=theta_new
