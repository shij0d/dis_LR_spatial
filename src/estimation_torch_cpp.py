...
def ce_optimize_stage2_cpp(locs_list, z_list, X_list, knots,
    ...
    def _f64(t):
        if isinstance(t, (int, float)):
            return torch.tensor([float(t)], dtype=torch.float64)
        return t.to(dtype=torch.float64).contiguous()

    locs_vec = [_f64(t) for t in locs_list]
    z_vec    = [_f64(t) for t in z_list]
    X_vec    = [_f64(t) if (isinstance(t, torch.Tensor) and t.numel() > 0)
                else torch.empty(0, dtype=torch.float64) for t in X_list]
    knots_t  = _f64(knots)
    mu0_t    = _f64(mu0).reshape(-1, 1)
    Sigma0_t = _f64(Sigma0)
    beta0_t  = _f64(beta0).reshape(-1, 1)
    delta0_t = _f64(delta0).reshape(-1)
    theta0_t = _f64(theta0).reshape(-1)
