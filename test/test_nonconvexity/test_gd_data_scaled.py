...
for length_scale in length_scales:
    ...
    dis_data_scaled=[]
    for local_data in dis_data:
        local_data_scaled=local_data.copy()
        local_data_scaled[:,0:2]=local_data_scaled[:,0:2]*scale
        dis_data_scaled.append(local_data_scaled)
        
    knots=data['knots']
    knots_scaled=[]
    for knot in knots:
        knot_scaled=(knot[0]*scale,knot[1]*scale)
        knots_scaled.append(knot_scaled)
    ...
    gpp_estimation = GPPEstimation(dis_data_scaled, exponential_kernel, knots_scaled, weights_full)
