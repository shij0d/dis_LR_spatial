...
from src.generation import GPPSampleGeneratorUnitSquare
...
def estimate(r,N):
    ...
    sampler=GPPSampleGeneratorUnitSquare(num=N,kernel=kernel,coefficients=coefficients,noise=noise_level,seed=r)
    data,knots=sampler.generate_obs_gp(m=100,method="grid")
    ...
    T=100
    try:
        verbose_setting={'frequency':1, 'print_inner': True} #you can change the verbose setting here
        de_estimators=gpp_estimation.de_optimize_stage2(mu_list,Sigma_list,beta_list,delta_list,theta_list,T=T,weights_round=6,verbose_setting=verbose_setting)
