...
length_scales=[0.1,0.03]
rs=[r for r in range(100)]
for length_scale in length_scales:
    print(f"length_scale:{length_scale}")
    estimate_l=partial(estimate,length_scale=length_scale)
    results=[]
    for r in rs:
        print(f"r:{r}")
        result=estimate_l(r)
        results.append(result)
    with open(f'/home/shij0d/Documents/Dis_Spatial/expriements/centralized/exp1_parameter_length_scale_{length_scale}.pkl', 'wb') as f:
        pickle.dump(results, f)
    # with multiprocessing.Pool(processes=num_cores//4*3) as pool:
